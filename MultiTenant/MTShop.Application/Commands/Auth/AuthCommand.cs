...
namespace MTShop.Application.Commands.Auth
{
    ...
    public class AuthCommandHandler : IRequestHandler<AuthCommand, AuthResponseDTO>
    {
        ...
        public AuthCommandHandler(IIdentityService identityService, IConfiguration configuration)
        {
        ...
        public async Task<AuthResponseDTO> Handle(AuthCommand request, CancellationToken cancellationToken)
        {
            ...
            var (userId, userName, tenantId, roles) = await _identityService.GetUserDetailsAsync(userIdByName);

            var (token, expirationTime) = GenerateJwtToken((userId, userName, tenantId, roles));
        ...
        private (string, DateTime) GenerateJwtToken((string userId, string userName, string tenantId, IList<string> roles) userDetails)
        {
            var securityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_configuration["Jwt:Key"]));
            var credentials = new SigningCredentials(securityKey, SecurityAlgorithms.HmacSha256);

            var (userId, userName, tenantId, roles) = userDetails;
            var claims = new List<Claim>()
            {
                new Claim(JwtRegisteredClaimNames.Sub, userName),
                new Claim(JwtRegisteredClaimNames.Jti, tenantId),
